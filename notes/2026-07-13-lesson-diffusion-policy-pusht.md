...
- [x] Watch `eval_episode_1.mp4` (the one failure, reward 0.0) — did it get stuck, overshoot, or oscillate? Raw visual intuition before any instrumentation exists. — done, my own words: **"The T got stuck in the lower bottom corner and the grabber lost the ability to manipulate it."** Plausible mechanism: the pusher can only push, not grasp (established earlier in this lesson) — once the T wedges against two walls at once in a corner, there may be no remaining approach angle the pusher can reach that pushes it back toward open space. This is exactly the kind of failure raw video can suggest but not confirm — no instrumentation exists yet to show the actual pusher trajectory or contact geometry in that corner, which is precisely the Module 4 gap.
